...
export const Login: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<'welcome' | 'login' | 'register'>('login');
  ...
  return (
    <div className={cn(
      ...
      currentView === 'welcome' ? "bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#334155]" : "bg-gradient-to-br from-orange-50 via-white to-orange-50"
    )}>
      {/* Background Curves */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
        <svg className="absolute w-full h-full" viewBox="0 0 400 800" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M-50 -50 C 150 100 300 -50 450 150" stroke="#F95A2C" strokeOpacity="0.08" strokeWidth="1.5" fill="none" />
          <path d="M-50 200 C 100 400 -50 600 200 850" stroke="#F95A2C" strokeOpacity="0.08" strokeWidth="1.5" fill="none" />
          <path d="M450 400 C 250 500 350 700 150 900" stroke="#F95A2C" strokeOpacity="0.08" strokeWidth="1.5" fill="none" />
          <path d="M-100 700 C 100 600 300 800 500 700" stroke="#F95A2C" strokeOpacity="0.08" strokeWidth="1.5" fill="none" />
        </svg>
      </div>
            ...
            >
              ...
              <div className="relative z-10 space-y-4 mb-20">
                ...
                <p className="text-slate-300 text-sm px-6 leading-relaxed font-medium">
                  {t('Expert local professionals for all your home needs, repairs and rentals.')}
                </p>
              </div>
            ...
            >
              <div className="text-center mb-12">
                <div className="flex items-center justify-center gap-2 mb-8">
                  ...
                  <h1 className="text-xl font-black text-slate-900 tracking-tight">{t('DIP Services')}</h1>
                </div>
                <h1 className="text-3xl font-extrabold text-[#F95A2C] mb-6">{t('Login here')}</h1>
          ...
          {currentView === 'register' && (
            ...
            >
              <div className="text-center mb-8">
                ...
                <h1 className="text-3xl font-extrabold text-[#F95A2C] mb-4">{t('Create Account')}</h1>
                <p className="text-slate-500 text-sm max-w-[280px] mx-auto leading-relaxed font-medium">
                  {t('Create an account so you can explore all the existing services')}
                </p>
              </div>
              <form onSubmit={handleSubmit} className="space-y-4 flex-1 flex flex-col">
                ...
                />
                  <div className="relative group">
                    ...
                    <button
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-[#F95A2C] hover:scale-110 active:scale-95 transition-all p-1"
                      title={t('Pick on Map')}
                    >
                      <MapPin size={22} />
                    </button>
                ...
                <div className="space-y-3 pt-2 pb-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">{t('I want to')}</label>
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <button
                      type="button"
                      onClick={() => setRole('user')}
                      className={cn(
                        "w-full py-4 rounded-xl border-2 transition-all flex flex-row items-center justify-center gap-4 px-6",
                        role === 'user' 
                          ? "border-[#F95A2C] bg-[#F95A2C]/10 text-[#F95A2C] shadow-sm" 
                          : "border-transparent bg-[#F8F9FA] text-slate-500 hover:bg-slate-100"
                      )}
                    >
                      <UserCircle size={24} className={role === 'user' ? "text-[#F95A2C]" : "text-slate-400"} />
                      <span className="text-sm font-bold uppercase tracking-wide">{t('Hire Services')}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Hammer size={24} className={role === 'worker' ? "text-[#F95A2C]" : "text-slate-400"} />
                      <span className="text-sm font-bold uppercase tracking-wide">{t('Provide Services')}</span>
                    </button>
                  </div>
                </div>
