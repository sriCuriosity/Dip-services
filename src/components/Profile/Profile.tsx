...
import { LogOut, User, Phone, MapPin, Mail, Camera, ChevronRight, ShieldCheck, Briefcase, Settings, Loader2, Car, Repeat, ChevronDown, ChevronUp, Edit2, Building2, Clock, X, Users, Heart, Download, Globe, CreditCard, Monitor, Trash2, History, ChevronLeft, MessageSquare, Info, PlayCircle } from 'lucide-react';
...
export const Profile: React.FC = () => {
  ...
  useEffect(() => {
    const handlePopState = () => {
      setIsChatOpen(!!window.history.state?.chat);
      setIsSettingsOpen(!!window.history.state?.settings);
      setIsEditing(!!window.history.state?.editing);
    ...
    if (window.history.state?.chat) setIsChatOpen(true);
    if (window.history.state?.settings) setIsSettingsOpen(true);
    if (window.history.state?.editing) setIsEditing(true);
  ...
  const handleSetPreviewImage = (image: { src: string, alt: string } | null) => {
    if (image) window.history.pushState({ preview: true }, '');
  ...
  const handleSwitchRole = async () => {
    ...
    if (profile.role === 'user') {
      ...
      await update(ref(db, `users/${profile.uid}`), { role: 'worker' });
    } else {
      await update(ref(db, `users/${profile.uid}`), { role: 'user' });
    }
  ...
  const handleDeleteTrade = async (tradeName: string) => {
    if (!profile || !window.confirm(t('Are you sure you want to delete this service?'))) return;
  ...
  const ProfileOption = ({ icon: Icon, label, onClick, color = "text-slate-700", subLabel }: { icon: any, label: string, onClick?: () => void, color?: string, subLabel?: string }) => (
    <button 
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-3.5 flex-1 min-w-0">
        ...
        <div className="text-left flex-1 min-w-0 py-1">
          <span className={cn("font-semibold text-[14px] block break-words leading-tight", color)}>{label}</span>
          {subLabel && <span className="text-[11px] text-slate-400 block mt-0.5 break-words leading-tight">{subLabel}</span>}
        </div>
      </div>
      <ChevronRight size={16} className="text-slate-300 group-hover:text-slate-400 group-hover:translate-x-0.5 transition-all shrink-0 ml-2" />
    </button>
  );
    ...
    return (
      <div className="flex flex-col min-h-full bg-slate-50/50">
        <div className="px-6 pt-12 pb-6 flex items-center justify-between bg-white border-b border-slate-100 sticky top-0 z-20">
          <button onClick={() => window.history.back()} className="p-2 -ml-2 text-slate-900 hover:bg-slate-50 rounded-full transition-colors">
            <ChevronLeft size={24} />
          </button>
          <h1 className="text-lg font-bold text-slate-900">{t('Edit Profile')}</h1>
          <div className="w-10" />
        </div>
          ...
          <div className="flex flex-col items-center gap-4 mb-8">
            <div className="relative group">
              <div 
                ...
                onClick={() => {
                  if (localPhotoURL) {
                    handleSetPreviewImage({ src: localPhotoURL, alt: profile?.name || 'Profile' });
                  }
                }}
              >
                {uploading ? (
                  ...
                  <img src={localPhotoURL} alt={profile?.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                ) : (
            ...
            {uploadSize && <p className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{uploadSize}</p>}
          </div>

          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-5">
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">{t('Full Name')}</label>
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">{t('Phone Number')}</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">{t('City')}</label>
              <div className="relative">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider ml-1">{t('Address')}</label>
              </div>
              <div className="relative">
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">{t('Landmark')}</label>
              <div className="relative">
                <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} />
                <input
                  type="text"
    ...
    <div className="flex flex-col min-h-full bg-slate-50/30">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-900 pt-[calc(1rem+env(safe-area-inset-top,0px))] pb-4 px-3.5 rounded-b-[2.5rem] shadow-2xl shadow-blue-200/40">
        ...
        <div className="relative z-10 flex items-center justify-between mb-1">
          <button onClick={() => navigate(-1)} className="p-2.5 -ml-2 text-white/90 hover:text-white hover:bg-white/10 rounded-full transition-all active:scale-90">
            <ChevronLeft size={22} />
          </button>
          <h1 className="text-sm font-black text-white/90 tracking-[0.25em] uppercase">{t('My Profile')}</h1>
          <button onClick={() => handleOpenSettings()} className="p-2.5 -mr-2 text-white/90 hover:text-white hover:bg-white/10 rounded-full transition-all active:scale-90">
            <Settings size={22} />
          </button>
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center sm:flex-row sm:text-left sm:items-center gap-1.5">
          ...
          >
            {localPhotoURL ? (
              <img src={localPhotoURL} alt={profile?.name} className="w-full h-full object-cover transition-transform group-hover:scale-110" referrerPolicy="no-referrer" />
            ) : (
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
              <Camera size={20} className="text-white opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all" />
            </div>
          </div>

          <div className="flex-1 min-w-0 flex flex-col gap-1.5 py-0.5">
            <h2 className="text-lg font-black text-white break-words tracking-[-0.05em] mb-0 drop-shadow-sm leading-tight">{profile?.name}</h2>
            <p className="text-blue-100/70 text-[10px] break-words mb-0 font-semibold tracking-normal leading-normal">{profile?.email}</p>
            <div className="flex items-center justify-start gap-1.5 mt-0.5">
              ...
              {hasWorkerProfile && (
                <button 
                  ...
                  className="bg-white/10 backdrop-blur-md text-white px-2 py-1.5 rounded-lg text-[9px] font-black active:scale-95 transition-all flex items-center gap-1 border border-white/20 hover:bg-white/20 shadow-lg uppercase tracking-wide whitespace-nowrap"
                >
                  <Repeat size={10} />
                  {profile?.role === 'user' ? t('Switch to Worker') : t('Switch to User')}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="px-6 py-6 pb-32 space-y-6">
        {profile?.role === 'worker' && (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <p className="font-bold text-slate-900 text-[15px]">{t('Service Profile')}</p>
                    <p className="text-slate-400 text-[11px] font-medium">{t('Manage your trade and rates')}</p>
                  </div>
                </div>
              ...
              {workerData?.trades && (
                <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider">{t('My Services')}</h2>
                    <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{Object.keys(workerData.trades).length} {t('Active')}</span>
                  </div>
                  <div className="space-y-3">
                    {Object.entries(workerData.trades).map(([tradeName, tradeData]: [string, any]) => (
                      <div key={tradeName} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50 hover:bg-white hover:border-blue-100 transition-all group gap-3">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-slate-800 text-sm break-words">{t(tradeName)}</h3>
                          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-tight">{tradeData.experience} {t('Experience')}</p>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                          <div className={cn(
                            "px-2 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider flex items-center gap-1 shrink-0",
                            tradeData.verificationStatus === 'approved' ? "bg-emerald-100 text-emerald-700" :
                            tradeData.verificationStatus === 'rejected' ? "bg-red-100 text-red-700" :
                            "bg-amber-100 text-amber-700"
                          )}>
                            {tradeData.verificationStatus === 'approved' ? <ShieldCheck size={10} /> : 
                             tradeData.verificationStatus === 'rejected' ? <X size={10} /> : 
                             <Clock size={10} />}
                            {t(tradeData.verificationStatus === 'approved' ? 'Verified' : 
                               tradeData.verificationStatus === 'rejected' ? 'Rejected' : 'Pending')}
                          </div>
                          <button 
                            onClick={() => navigate(`/worker/profile/${tradeName}`)}
              ...
              <button 
                onClick={() => navigate('/profile/shop-setup')}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <p className="font-bold text-slate-900 text-[15px]">{t('Shop Profile')}</p>
                    <p className="text-slate-400 text-[11px] font-medium">{t('Manage your shop')}</p>
                  </div>
                </div>
                <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center">
                  <ChevronRight size={18} className="text-slate-300" />
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden p-2">
            {profile?.role !== 'worker' && (
              <>
                <ProfileOption icon={Building2} label={t('Shops')} onClick={() => navigate('/shops')} subLabel={t('Browse local businesses')} />
                <div className="h-[1px] bg-slate-50 mx-4" />
              </>
            )}
            <ProfileOption icon={Users} label={t('Members')} onClick={() => navigate('/members')} subLabel={t('Community members')} />
            <div className="h-[1px] bg-slate-50 mx-4" />
            <ProfileOption icon={MessageSquare} label={t('Chat with Admin')} onClick={() => handleOpenChat()} subLabel={t('Contact support team')} />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden p-2">
            <ProfileOption icon={PlayCircle} label={t('Helping Video')} subLabel={t('Watch how to use the app')} />
            <div className="h-[1px] bg-slate-50 mx-4" />
            <ProfileOption 
              icon={LogOut} 
              label={t('Logout')} 
